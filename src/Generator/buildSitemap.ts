...
async function main() {
  ...
  if (!fs.existsSync(ARTICLES_ROOT)) {
    console.warn(`No articles directory found at ${ARTICLES_ROOT}. Writing sitemap with only static routes.`);
  } else {
    for (const f of walk(ARTICLES_ROOT)) {
      if (f.endsWith('.json')) articleFiles.push(f);
    }
  }
  ...
  const staticRoutes: { loc: string; changefreq: ChangeFreq; priority: number; lastmod: string }[] = [
    { loc: `${SITE_URL}`, changefreq: 'daily', priority: 0.7, lastmod: nowISO },
    { loc: `${SITE_URL}/Blog`, changefreq: 'daily', priority: 0.7, lastmod: nowISO },
  ];
  ...
  const articleBlocks = articles
    .sort((a, b) => a.city.localeCompare(b.city) || a.slug.localeCompare(b.slug))
    .map(a =>
